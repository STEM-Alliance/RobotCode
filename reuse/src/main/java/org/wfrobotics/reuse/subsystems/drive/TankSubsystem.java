...
public final class TankSubsystem extends EnhancedSubsystem
{
    ...
    static
    {
        final TankConfig config = IRobotConfig.getInstance().getTankConfig();
    ...
    {
        setDefaultCommand(IRobotConfig.getInstance().getTankConfig().getTeleopCommand());
    }
    ...
    private void updatePathFollower()
    {
        if (controlMode == TANK_MODE.PATH_2018)
        {
            ...
            DriveMotionPlanner.Output output = mMotionPlanner.update(now, IRobotState.getInstance().getFieldToVehicle(now));
